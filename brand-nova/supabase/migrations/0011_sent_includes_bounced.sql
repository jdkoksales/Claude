-- "Sent" must mean "actually left the building", including mails that later
-- bounced (status flips sent -> bounced). Filtering on status='sent'
-- undercounted sends by the number of bounces, skewing bounce/open rates and
-- (in the tick) letting the daily cap overshoot by the day's bounces.
...
as $$
  select json_build_object(
    'sent',       count(s.id) filter (where s.sent_at is not null),
...
select
  ...
  count(s.id) filter (where s.sent_at is not null)                as sent,
